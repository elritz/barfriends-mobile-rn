...
export default () => {
  ...
  const [expoPushNotificationToken, setExpoPushNotificationToken] =
    useState("");
        ...
        return (
          <Pressable key={index} onPress={item.onPress} className="mx-3">
            {({ isPressed }) => {
              return (
                <Box
                  className={`h-[${ITEM_HEIGHT}px] ${isPressed ? "bg-light-100" : "bg-transparent"} ${isPressed ? "dark:bg-light-800" : "dark:bg-transparent"} justify-between`}
                >
                  <Divider />
        ...
        return (
          <Pressable key={index} onPress={item.onPress} className="mx-3">
            {({ pressed }) => {
              return (
                <Box
                  className={` h-[${ITEM_HEIGHT}] ${pressed ? "bg-light-100" : "bg-transparent"} ${pressed ? "dark:bg-light-800" : "dark:bg-transparent"} justify-between`}
                >
                  <Divider />
                  <HStack
                    space={"md"}
            ...
            {({ pressed }) => {
              return (
                <VStack>
                  <Divider />
                  <Heading className="text-lg">{item.title}</Heading>
                  <HStack className="items-center justify-between rounded-lg py-4">
                    <Text
                      ellipsizeMode={"tail"}
                      numberOfLines={1}
