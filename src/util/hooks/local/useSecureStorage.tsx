...
const secureStorageItemCreate = async ({
  ...
  value,
  options,
}: CreateTokenProps): Promise<void> =>
  SecureStore.setItemAsync(key, value, options)

const secureStorageItemDelete = async ({
  key,
  options,
}: DeleteTokenProps): Promise<void> => SecureStore.deleteItemAsync(key, options)

export {secureStorageItemCreate, secureStorageItemDelete, secureStorageItemRead}
